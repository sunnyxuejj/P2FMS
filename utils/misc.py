...
def args_parser():
    ...
    parser.add_argument('--type', type=str, default='sms', help='which kind of wireless traffic')
    parser.add_argument('--esm_m', type=str, default='ets', help='esm method')
    ...
    parser.add_argument('--client_all_epoch', type=int, default=5,
                        help='the number of local personal updating epochs')
    parser.add_argument('--personal_epoch', type=int, default=2,
                        help='the number of local personal updating epochs')
    parser.add_argument('--ft_epoch', type=int, default=6,
                        help='the number of local fine-tuning epochs')
    parser.add_argument('--local_bs', type=int, default=20,
                        help='local batch size: B')
    parser.add_argument('--epsilon', type=float, default=1, help='stepsize')
    parser.add_argument('--agg', type=str, default='avg', help='parameter aggregation')
    parser.add_argument('--phi', type=float, default=100.0, help='how many samples are shared')
    ...
    parser.add_argument('--linear_hidden', type=int, default=1,
                        help='hidden neurons of esm linear layer')
    ...
    parser.add_argument('--dropout_hh', type=float, default=0.3,
                        help='amount of weight dropout to apply to the RNN hidden to hidden matrix')
    parser.add_argument('--lam_ditto', type=float, default=10, help='Ditto parameter lambda')
    parser.add_argument('--alpha_l2gd', type=float, default=0.05, help='l2gd parameter alpha')
    parser.add_argument('--lambda_l2gd', type=float, default=3, help='l2gd parameter lambda')
    parser.add_argument('--alpha_apfl', type=float, default=0.8, help='APFL parameter alpha')
...
def plot_predict(args, col, data, pred, method):
    plt.figure(figsize=(12, 9))
    plt.plot(data, label="observed")
    plt.plot(pred, label="pred")
    plt.legend()
    plt.title(f"{method}_prediction")
    plt.savefig('{}_plot/{}_{}/predict_{}.png'.format(args.esm_m, args.file, args.type, col))
    plt.show()
...
def process_isolated(args, dataset):
    ...
    for col in column_names:
        ...
        test_len = args.test_days * 24
        val_len = args.val_days * 24
        train_len = len(cell_traffic) - test_len - val_len
