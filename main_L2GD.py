...
class LocalUpdateL2GD(object):
    ...
    def train(self, model, lr, local_epoch):
        model.train()
        # train and update
...
def train_L2GD(args, selected_cells, train, val, device, model_saved):
    ...
    w_locals_save, w_locals, val_loss_p = {}, {}, {}
    ...
    for epoch in range(args.epochs + 1):
        ...
        if u > p:
            for idx, cell in enumerate(cell_idx):
                ...
                local = LocalUpdateL2GD(args, cell_train, cell_val, cell_test, idx, cell)

                net_local = copy.deepcopy(global_model)
                net_local.load_state_dict(w_locals[cell])
                w_local, loss = local.train(model=net_local.to(device), lr=lr, local_epoch=args.local_epoch)
                ...
                if val_loss_p_cell < val_loss_p[cell]:
                    w_locals_save[cell] = copy.deepcopy(w_local)
                    val_loss_p[cell] = copy.deepcopy(val_loss_p_cell)

            print('Round: {}, Average training loss of global model: {:.4f}'.format(epoch, sum(local_losses) / len(local_losses)))

        else:
            for idx, cell in enumerate(cell_idx):
                if idx == 0:
                    w_glob = copy.deepcopy(w_locals[cell])
                    for k, key in enumerate(global_model.state_dict().keys()):
                        w_glob[key] = w_glob[key] / m
                else:
                    for k, key in enumerate(global_model.state_dict().keys()):
                        w_glob[key] += w_locals[cell][key] / m
            global_model.load_state_dict(w_glob)
            # take step closer to average
            for idx, cell in enumerate(cell_idx):
                cell_val = val[cell]
                for k, key in enumerate(global_model.state_dict().keys()):
                    w_locals[cell][key] = (1 - lr*lam) * w_locals[cell][key] + lr*lam*w_glob[key]
                net_local = copy.deepcopy(global_model)
                net_local.load_state_dict(w_locals[cell])
                val_loss_p_cell, _, _, _, _ = test_inference(args, net_local, cell_val)
                if val_loss_p_cell < val_loss_p[cell]:
                    w_locals_save[cell] = copy.deepcopy(w_locals[cell])
                    val_loss_p[cell] = copy.deepcopy(val_loss_p_cell)
        ...
        print('Global Val loss: {:4f}, Val R2 score: {:4f}'.format(avg_val_loss, r2_score), flush=True)
...
if __name__ == '__main__':
    ...
    w_locals = train_L2GD(args, selected_cells, train, val, device, model_saved)
